// ====================================================================
//  effects.h - LED animation effects + static pixel patterns
//
//  Effect state variables, the per-effect update*() functions, the
//  pattern-generation API (/api/pixels/set), resetEffectState(),
//  initAllEffects() and renderEffect(). Everything here talks to the
//  strip via the shared globals declared in globals.h.
// ====================================================================
#pragma once
...
// Validate a hex color string: 3 or 6 hex digits, case-insensitive.
bool isValidHexColor(const String& hex) {
  String h = hex;
  h.trim();
  if (h.length() != 3 && h.length() != 6) return false;
  for (int j = 0; j < (int)h.length(); j++) {
    char c = h.charAt(j);
...
// One-time initialization of every effect's state. Called from setup().
// Per-effect lazy inits (initTwinkle, initFireFlicker, initBalls, ...)
// still run on demand inside each update function.
void initAllEffects() {
  // Fireflies
  for (int i=0; i<MAX_FIREFLIES; i++) fireflies[i].active=false;
...
// Draw one frame of the currently selected effect. Called from loop().
void renderEffect() {
